...
export function DraftPlayLogo({ size = 40, color = "#3D9968", animate = false }: DraftPlayLogoProps) {
  ...
  // Use a unique filter ID per instance to avoid cross-component reference bugs
  const filterId = `dp-shadow-${useId().replace(/:/g, "")}`;
  if (Platform.OS === "web") {
    return (
      <YStack width={size} height={size} alignItems="center" justifyContent="center" testID="draftplay-logo">
        ...
        <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Bowl arc — renders without filter first, shadow is enhancement only */}
          <path d="M18.1 17.7 A8.35 8.35 0 0 0 18.1 34.4" stroke={color} strokeWidth="2.8" fill="none" strokeLinecap="round" filter={`url(#${filterId})`}/>
          {/* Vertical bar */}
          <rect x="18.1" y="5.6" width="2.8" height="30.2" fill={color} filter={`url(#${filterId})`}/>

          {/* Play triangle — animated */}
          <g className={animate ? "dp-logo-click-group" : undefined}>
            <polygon points="20.9,5.6 30.26,12.8 20.9,20.0" fill={color} filter={`url(#${filterId})`}/>
          </g>
